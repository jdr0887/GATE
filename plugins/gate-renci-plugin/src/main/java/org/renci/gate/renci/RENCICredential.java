...
public class RENCICredential implements RSPCredential {

    private static final long serialVersionUID = 1L;

    protected static final int DEFAULT_LIFETIME = 12 * 60 * 60; // 12 hours

    private final Logger logger = LoggerFactory.getLogger(RENCICredential.class);

    private final Executor exec = Executor.getInstance();

    protected File jobDir;

    private String proxyFileName = ".renci_proxy";
    ...
    public RENCICredential(BundleContext context) {
        super();
    ...
    public void create(Dictionary<Object, Object> properties) throws Exception {
        ...
        logger.info("Renewing RENCI credential in " + jobDir);
        ...
        File in = File.createTempFile("gate-renci", "pwd");
        ...
        command.append("myproxy-logon --pshost ").append(myproxyHost).append(" --psport ").append(myproxyPort)
                ...
                .append(File.separator).append(getProxyFileName()).append(" --stdin_pass <").append(in.getPath());
