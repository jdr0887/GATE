...
public class GATESite {
    ...
    private RSPCredential credential;
    ...
    public void setCredential(RSPCredential credential) {
    ...
    public int getScore() {
        ...
        if (totalCount >= maxTotalCount) {
            scoreComment = "Total number of glideins has reached the limit of " + maxTotalCount;
            return 0;
        }
        if (idleCount >= maxIdleCount) {
            scoreComment = "Number of idle glideins has reached the limit of " + maxIdleCount;
            return 0;
        }
        ...
        // one or a few jobs
        if (totalCount == 0) {
            scoreComment = "Total number of glides is 0";
            // also set maxsubmit to 1
            maxMultipleJobs = 1;
            return 500;
        }
        ...
        // held jobs are bad
        score = score - heldCount * 6;

        // idle jobs are not so good
        score = score - idleCount * 3;

        // running jobs are good
        score = score + runningCount;
        ...
        if (score > 15 && runningCount > 10 && idleCount < 5) {
            scoreComment = "Score lowered to spread jobs out on available sites";
            score = 1;
        }
