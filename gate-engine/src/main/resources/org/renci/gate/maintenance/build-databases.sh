...
if [ -e "$APPLICATION_DIR/bin/formatdb" ]; then

    echo
    mkdir -p $DATABASE_DIR/renci/update-databases
    cd $DATABASE_DIR/renci/update-databases

    wget -q -O run-all http://beluga.renci.org/databases/renci/update-databases/run-all
    chmod 755 run-all
    ./run-all
fi
